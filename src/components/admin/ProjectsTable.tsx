...
export default function ProjectsTable({ initialProjects }: { initialProjects: ProjectRecord[] }) {
  ...
  const publishedBadgeClass = (published: boolean) =>
    published
      ? "text-[11px] tracking-wider uppercase text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-full px-2.5 py-1 shrink-0"
      : "text-[11px] tracking-wider uppercase text-slate-400 bg-slate-50 border border-slate-200 rounded-full px-2.5 py-1 shrink-0";
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      ...
      <div className="lg:hidden divide-y divide-slate-100">
        {projects.map((p, i) => (
          <div key={p.id} className={cn("px-4 py-4", pending && "opacity-60")}>
            <div className="flex gap-3">
              <Link href={`/admin/projects/${p.id}/edit`} className="relative w-16 h-14 rounded-lg overflow-hidden bg-slate-100 shrink-0">
                {p.heroImage && <Image src={p.heroImage} alt="" fill sizes="64px" className="object-cover" />}
              </Link>
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <Link
                    href={`/admin/projects/${p.id}/edit`}
                    className="font-medium text-[#1C2420] text-sm leading-snug truncate"
                  >
                    {p.title}
                  </Link>
                </div>
                ...
                <div className="flex items-center justify-between mt-2.5">
                  <div className="flex items-center gap-3">
                    <div className="flex gap-2.5">
                      <button
                        ...
                        disabled={i === 0}
                        onClick={() => move(i, -1)}
                        className="text-slate-400 hover:text-[#1C2420] disabled:opacity-20 text-sm leading-none"
                        aria-label="Move up"
                      >
                        ▲
                      </button>
                    ...
                    <Link href={`/admin/projects/${p.id}/edit`} className="text-slate-500 hover:text-[#1C2420] text-xs">
                      Edit
                    </Link>
                    <button
                      type="button"
                  ...
                  <button type="button" onClick={() => handleToggle(p.id, !p.published)} className={publishedBadgeClass(p.published)}>
